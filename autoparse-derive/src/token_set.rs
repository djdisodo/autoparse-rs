...
pub fn derive_token_set(input: TokenStream2) {
		...
		syn::Data::Enum(data_enum) {
			for variant in data_enum.variants {
				if let Fields::Unnamed(fields_unnamed) = variant.fields {
					...
					let field = fields.next().expect("enum deriving TokenSet cannot have variant with no field");
					let mut output_token_stream = TokenStream2::new();
					let ty = field.ty;
